...
在master分支以外，再建立不同的环境分支。比如，"开发环境"的分支是master，"测试环境"的分支是pre-production，"生产环境"的分支是production。
...
- 功能需求或bug修复，从master拉出新feature分支，命名为**feature/{开发人员姓名首字母缩写}-{修改主题}**。在发布完成后，删除分支。
